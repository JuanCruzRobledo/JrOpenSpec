...
interface Props {
  ...
  categories: Category[];
  /** Available subcategories — will be filtered by selected category */
 ...
 * Price input is in pesos — converted to cents for API.
 */
export function ProductForm({
  ...
  // Controlled state for category-subcategory cascading filter
  const [selectedCategoryId, setSelectedCategoryId] = useState<number | null>(
    product?.categoria_id ?? null,
  );

  // Controlled state for description character counter
  const [descripcion, setDescripcion] = useState(product?.descripcion ?? '');

  // Controlled price in pesos for display
  ...
  const submitAction = useCallback(
    async (_prevState: FormState, formData: FormData): Promise<FormState> => {
      ...
      // Convert pesos to cents — Math.round to avoid floating point issues
      ...
      try {
        ...
        if (isEditing && product) {
          await updateFn(product.id, data);
        } else {
          await createFn(data);
        }
