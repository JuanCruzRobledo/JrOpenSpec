"""FastAPI dependencies — auth, tenant, and infrastructure injection."""
...
from shared.security.table_token import verify_table_token as _verify_table_token
...
# ---------------------------------------------------------------------------
# JWT helpers (imported lazily to avoid circular deps with Batch A code)
# ---------------------------------------------------------------------------
...
async def get_optional_user(
    ...
    anonymous users (e.g. public menu with optional personalization).
    """
    auth_header = request.headers.get("Authorization")
    if not auth_header or not auth_header.startswith("Bearer "):
        return None
...
def require_roles(*roles: str) -> Callable:
    ...
    Usage::
        ...
        async def admin_view(user: dict = Depends(require_roles("ADMIN"))):
            ...
    """
...
async def verify_table_token_dep(request: Request) -> dict[str, Any]:
    ...
    Returns:
        Dict with branch_id, table_id, session_id, exp.

    Raises:
        AuthenticationError: On missing/invalid/expired token.
    """
