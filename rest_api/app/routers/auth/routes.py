...
from rest_api.app.middleware.rate_limit import build_login_rate_limit_response
...
async def login(
    ...
    try:
        ...
        return build_login_rate_limit_response(exc.retry_after or 1)
...
async def refresh(
    ...
    try:
        new_access_token, new_refresh_token = await auth_service.refresh(
            refresh_token,
            current_access_token=_extract_bearer_token(request),
        )
    except AuthenticationError as exc:
...
async def logout(
    ...
    try:
        await auth_service.logout(token, refresh_token_str=refresh_token)
    except AuthenticationError as exc:
        return JSONResponse(status_code=401, content={"detail": exc.message})
