"""Category router — CRUD endpoints nested under /api/v1/branches/{branch_id}/categories.

Thin handlers: Depends + PermissionContext + service call.
"""

from __future__ import annotations

import logging
from typing import Any

from fastapi import APIRouter, Depends, status
from sqlalchemy.ext.asyncio import AsyncSession

from rest_api.app.dependencies import get_current_user
from rest_api.app.schemas.category import (
    CategoryCreate,
    CategoryDeleteResponse,
    CategoryRead,
    CategoryUpdate,
)
from rest_api.app.schemas.envelope import ListResponse, SingleResponse
from rest_api.app.services.domain.category_service import CategoryService
from rest_api.app.services.permissions.context import PermissionContext
from shared.infrastructure.db import get_db

logger = logging.getLogger(__name__)

router = APIRouter(
    prefix="/branches/{branch_id}/categories",
    tags=["categories"],
)


def _get_service(db: AsyncSession = Depends(get_db)) -> CategoryService:
    return CategoryService(db)


@router.get("/", response_model=ListResponse[CategoryRead])
async def list_categories(
    branch_id: int,
    page: int = 1,
    limit: int = 10,
    user: dict[str, Any] = Depends(get_current_user),
    service: CategoryService = Depends(_get_service),
) -> dict:
    """GET /api/v1/branches/{branch_id}/categories?page=1&limit=10."""
    ctx = PermissionContext(user)
    ctx.require_management()
    ctx.require_branch_access(branch_id)

    items, total = await service.list(
        tenant_id=ctx.tenant_id,
        branch_id=branch_id,
        page=page,
        limit=limit,
    )
    return {
        "data": items,
        "meta": {"page": page, "limit": limit, "total": total},
    }


@router.post("/", response_model=SingleResponse[CategoryRead], status_code=status.HTTP_201_CREATED)
async def create_category(
    branch_id: int,
    body: CategoryCreate,
    user: dict[str, Any] = Depends(get_current_user),
    service: CategoryService = Depends(_get_service),
) -> dict:
    """POST /api/v1/branches/{branch_id}/categories."""
    ctx = PermissionContext(user)
    ctx.require_management()
    ctx.require_branch_access(branch_id)

    data = await service.create(
        tenant_id=ctx.tenant_id,
        branch_id=branch_id,
        data=body.model_dump(exclude_unset=True),
        user_id=int(ctx.user_id),
    )
    return {"data": data}


@router.put("/{category_id}", response_model=SingleResponse[CategoryRead])
async def update_category(
    branch_id: int,
    category_id: int,
    body: CategoryUpdate,
    user: dict[str, Any] = Depends(get_current_user),
    service: CategoryService = Depends(_get_service),
) -> dict:
    """PUT /api/v1/branches/{branch_id}/categories/{id}."""
    ctx = PermissionContext(user)
    ctx.require_management()
    ctx.require_branch_access(branch_id)

    data = await service.update(
        category_id=category_id,
        tenant_id=ctx.tenant_id,
        branch_id=branch_id,
        data=body.model_dump(exclude_unset=True),
        user_id=int(ctx.user_id),
    )
    return {"data": data}


@router.delete("/{category_id}", response_model=SingleResponse[CategoryDeleteResponse])
async def delete_category(
    branch_id: int,
    category_id: int,
    user: dict[str, Any] = Depends(get_current_user),
    service: CategoryService = Depends(_get_service),
) -> dict:
    """DELETE /api/v1/branches/{branch_id}/categories/{id}."""
    ctx = PermissionContext(user)
    ctx.require_admin()
    ctx.require_branch_access(branch_id)

    data = await service.delete(
        category_id=category_id,
        tenant_id=ctx.tenant_id,
        branch_id=branch_id,
        user_id=int(ctx.user_id),
    )
    return {"data": data}
