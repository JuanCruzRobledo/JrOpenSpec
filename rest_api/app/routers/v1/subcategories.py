"""Subcategory router — CRUD nested under /api/v1/branches/{branch_id}/subcategories.

Thin handlers: Depends + PermissionContext + service call.
"""

from __future__ import annotations

import logging
from typing import Any

from fastapi import APIRouter, Depends, status
from sqlalchemy.ext.asyncio import AsyncSession

from rest_api.app.dependencies import get_current_user
from rest_api.app.schemas.envelope import ListResponse, SingleResponse
from rest_api.app.schemas.subcategory import (
    SubcategoryCreate,
    SubcategoryDeleteResponse,
    SubcategoryRead,
    SubcategoryUpdate,
)
from rest_api.app.services.domain.subcategory_service import SubcategoryService
from rest_api.app.services.permissions.context import PermissionContext
from shared.infrastructure.db import get_db

logger = logging.getLogger(__name__)

router = APIRouter(
    prefix="/branches/{branch_id}/subcategories",
    tags=["subcategories"],
)


def _get_service(db: AsyncSession = Depends(get_db)) -> SubcategoryService:
    return SubcategoryService(db)


@router.get("/", response_model=ListResponse[SubcategoryRead])
async def list_subcategories(
    branch_id: int,
    page: int = 1,
    limit: int = 10,
    category_id: int | None = None,
    user: dict[str, Any] = Depends(get_current_user),
    service: SubcategoryService = Depends(_get_service),
) -> dict:
    """GET /api/v1/branches/{branch_id}/subcategories?page=1&limit=10&category_id={opt}."""
    ctx = PermissionContext(user)
    ctx.require_management()
    ctx.require_branch_access(branch_id)

    items, total = await service.list(
        tenant_id=ctx.tenant_id,
        branch_id=branch_id,
        page=page,
        limit=limit,
        category_id=category_id,
    )
    return {
        "data": items,
        "meta": {"page": page, "limit": limit, "total": total},
    }


@router.post("/", response_model=SingleResponse[SubcategoryRead], status_code=status.HTTP_201_CREATED)
async def create_subcategory(
    branch_id: int,
    body: SubcategoryCreate,
    user: dict[str, Any] = Depends(get_current_user),
    service: SubcategoryService = Depends(_get_service),
) -> dict:
    """POST /api/v1/branches/{branch_id}/subcategories."""
    ctx = PermissionContext(user)
    ctx.require_management()
    ctx.require_branch_access(branch_id)

    data = await service.create(
        tenant_id=ctx.tenant_id,
        branch_id=branch_id,
        data=body.model_dump(exclude_unset=True),
        user_id=int(ctx.user_id),
    )
    return {"data": data}


@router.put("/{subcategory_id}", response_model=SingleResponse[SubcategoryRead])
async def update_subcategory(
    branch_id: int,
    subcategory_id: int,
    body: SubcategoryUpdate,
    user: dict[str, Any] = Depends(get_current_user),
    service: SubcategoryService = Depends(_get_service),
) -> dict:
    """PUT /api/v1/branches/{branch_id}/subcategories/{id}."""
    ctx = PermissionContext(user)
    ctx.require_management()
    ctx.require_branch_access(branch_id)

    data = await service.update(
        subcategory_id=subcategory_id,
        tenant_id=ctx.tenant_id,
        branch_id=branch_id,
        data=body.model_dump(exclude_unset=True),
        user_id=int(ctx.user_id),
    )
    return {"data": data}


@router.delete("/{subcategory_id}", response_model=SingleResponse[SubcategoryDeleteResponse])
async def delete_subcategory(
    branch_id: int,
    subcategory_id: int,
    user: dict[str, Any] = Depends(get_current_user),
    service: SubcategoryService = Depends(_get_service),
) -> dict:
    """DELETE /api/v1/branches/{branch_id}/subcategories/{id}."""
    ctx = PermissionContext(user)
    ctx.require_admin()
    ctx.require_branch_access(branch_id)

    data = await service.delete(
        subcategory_id=subcategory_id,
        tenant_id=ctx.tenant_id,
        branch_id=branch_id,
        user_id=int(ctx.user_id),
    )
    return {"data": data}
